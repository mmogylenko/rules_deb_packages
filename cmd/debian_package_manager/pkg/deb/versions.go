...
func compareVersionStrings(va, vb string) (int, error) {
...
func parseVersion(verStr string) (Version, error) {
	...
	if len(splitOnColon) == 1 {
		...
		epoch, err := strconv.Atoi(splitOnColon[0])
		if err != nil {
...
func findDigits(str string) (digits int, restOfString string) {
	if str == "" {
		return 0, ""
	}
...
// returns remainder, non-digits.
func findNonDigits(str string) (nonDigitsStart, restOfString string) {
	if str == "" {
		return "", ""
	}
...
func compareRevisionStrings(a, b string) int {
	if a == b {
		return 0
	}
