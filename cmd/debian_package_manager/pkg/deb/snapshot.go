...
var (
	reSnapshot = regexp.MustCompile(`\d+T\d+Z`)
)
...
func Main(snapshot string, arch config.Arch, distro config.Distro) *PackageIndex {
	return &PackageIndex{
		URL: fmt.Sprintf(
			"https://snapshot.debian.org/archive/debian/%s/dists/%s/main/binary-%s/Packages.xz",
			snapshot, distro.Codename(), arch.DebianName(),
		),
...
func Updates(snapshot string, arch config.Arch, distro config.Distro) *PackageIndex {
	return &PackageIndex{
		URL: fmt.Sprintf(
			"https://snapshot.debian.org/archive/debian/%s/dists/%s-updates/main/binary-%s/Packages.xz",
			snapshot, distro.Codename(), arch.DebianName(),
		),
		PoolRoot: fmt.Sprintf("https://snapshot.debian.org/archive/debian/%s/", snapshot),
		Snapshot: snapshot,
...
func Security(snapshot string, arch config.Arch, distro config.Distro) *PackageIndex {
	packageURL := fmt.Sprintf(
		"https://snapshot.debian.org/archive/debian-security/%s/dists/%s-security/main/binary-%s/Packages.xz",
		snapshot, distro.Codename(), arch.DebianName(),
	)
	if distro == config.DEBIAN10 { // the url changed after debian10
		packageURL = fmt.Sprintf(
			"https://snapshot.debian.org/archive/debian-security/%s/dists/%s/updates/main/binary-%s/Packages.xz",
			snapshot, distro.Codename(), arch.DebianName(),
		)
	}
