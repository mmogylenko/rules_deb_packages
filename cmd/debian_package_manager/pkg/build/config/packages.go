...
type Config struct {
	DebianPackages PackagesYaml `yaml:"debian_packages"`
}
...
func validate(pjs []PackagesYaml) error {
	...
	for i, pj := range pjs {
		if len(pj.Archs) == 0 {
			err = fmt.Errorf("package config at index %d, Archs is empty", i)
		}

		if len(pj.Packages) == 0 {
			err = fmt.Errorf("package config at index %d, Packages is empty", i)
		}

		if len(pj.Distros) == 0 {
			err = fmt.Errorf("package config at index %d, Distros is empty", i)
		}
	}
...
func LoadPackages(path string) (Packages, error) {
	...
	pys := []PackagesYaml{cfg.DebianPackages}
