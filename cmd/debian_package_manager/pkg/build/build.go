...
import (
	...
	"github.com/mmogylenko/rules_deb_packages/cmd/debian_package_manager/pkg/build/config"
	"github.com/mmogylenko/rules_deb_packages/cmd/debian_package_manager/pkg/deb"
	"github.com/mmogylenko/rules_deb_packages/cmd/debian_package_manager/pkg/rhttp"
...
func extractPkgInfo(snapshots *config.Snapshots, arch config.Arch, distro config.Distro, packages map[string]bool) (
	mergedPackages map[string]*deb.Package, channels []string, err error) {
	pkgGrp := deb.PackageIndexGroup(snapshots, arch, distro)
	mergedPackages = map[string]*deb.Package{}
	channels = []string{}
	...
	for _, pix := range pkgGrp {
		...
		for pkg, pi := range rpi {
			existing, ok := mergedPackages[pkg]
			if (ok && deb.CompareVersion(existing.Version, pi.Version) == -1) || !ok {
				mergedPackages[pkg] = pi
			}
		}
	}

	if len(mergedPackages) != len(packages) {
		fmt.Println(len(mergedPackages), len(packages))

		var missing []string

		for pname := range packages {
			if _, ok := mergedPackages[pname]; !ok {
	...
	return mergedPackages, channels, nil
}
...
func checkForUpdates(current, latest *config.Snapshots, pkgDB config.Packages) (bool, error) {
	...
	for arch, distropackages := range pkgDB {
		...
		for distro, packages := range distropackages {
			latestVersions, _, err := extractPkgInfo(latest, arch, distro, packages)
			if err != nil {
				return false, err
			}
			...
			currentVersions, _, err := extractPkgInfo(current, arch, distro, packages)
			if err != nil {
				return false, err
			}
...
func writeConfig(snapshots *config.Snapshots, snapshotsFile string, pkgDB config.Packages, versionsFile, archivesFile string) error {
	...
	for arch, distropackages := range pkgDB {
		...
		for distro, packages := range distropackages {
			...
			errs.Go(func() error {
				packageInfo, channels, err := extractPkgInfo(snapshots, la, ld, lp)
				if err != nil {
	...
	fmt.Printf("Writing bazel file: %q\n", archivesFile)
...
func Update(snapshotsFile, packagesFile, versionsFile, archivesFile string) error {
...
func Generate(snapshotsFile, packagesFile, versionsFile, respositoriesFile string) error {
	var (
		snapshot, latest *config.Snapshots
		err              error
	)
